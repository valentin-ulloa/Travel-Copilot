...
def whatsapp_webhook(From: str = Form(...), Body: str = Form(...)):
    ...
    1) Validamos y normalizamos número.
    2) Si ya había “pendiente de fecha” para este número, procesamos la fecha y llamamos a AeroAPI.
    ...
    # ── 3) Intentamos traer el viaje del usuario (ya registrado) por teléfono
    trip = get_user_trip(From)
    if "error" not in trip:
        # El viaje ya está en la base → OMITIMOS AeroAPI. Vamos directo a OpenAI.
        user_ctx = (
    ...
    # ── 4) Si el usuario NO está en trips: detectamos flight_number
    posible_flight = detect_flight_pattern(Body)
    if posible_flight:
        # Le pedimos la fecha para ese vuelo dado que no existe en la base
        texto = (
    ...
    # ── 5) Ni usuario registrado ni flight_pattern → pedimos registro o localizador
    respuesta = (
